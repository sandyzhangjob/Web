...
class nacha_file
{
    ...
    public function __construct($origin_id, $company_id, $company_name, $settlement_routing_number, $settlement_account_number, $originating_bank_name, $balanced_file = false, $settlement_is_savings = false, $file_modifier = 'A')
    {
        ...
        if(date('w')==5){
            $settled_date= date('ymdhi',strtotime('+3 day'));
        } else {
            $settled_date= date('ymdhi',strtotime('+1 day'));
        }
        ...
        $this->file_header =    '1'.
                                ...
                                $this->format_number($origin_id, 10).
                                $this->format_number($settled_date,10).
                                //date('Hi'). //replace with "TEST" if testing
                                // "TEST". //uncomment in production
    ...
    private function create_batch($memo, $personal_payment = false, $service_class_code = '220')
    {
        if(!$this->open_file){
        ...
        if(date('w')==5){
            $settled_date= date('ymd',strtotime('+3 day'));
        } else {
            $settled_date= date('ymd',strtotime('+1 day'));
        }
        ...
        $batch_header =     '5'.
                            ...
                            $this->format_text('', 6).
                            $this->format_text($settled_date, 6).
                            $this->format_text('', 3).
                            '1'.
    ...
    {   
        ...
        if(!$this->numeric_only($bank_routing_to, 9)) $this->errors[] = 'Invalid credit routing number for transaction: '.$internal_id;
        if(!$this->numeric_only($bank_account_to, 17)) $this->errors[] = 'Invalid credit account number for transaction: '.$internal_id;
        if(!$this->alphameric_only($internal_id, 15)) $this->errors[] = 'Invalid internal ID for transaction: '.$internal_id;
        if(!$this->alphameric_only(substr($name, 0, 22), 22)) $this->errors[] = 'Invalid name for credit entry for transaction: '.$internal_id;
